 ...
 * Express app for all /api/* routes
 */
...
app.get('/api/questions', (req, res) => {
  try {
    ...
    const questionsWithOptions = questions.map(q => ({ ...q, options: allOptions.filter(o => o.questionId === q.id) }));
...
app.post('/api/questions', (req, res) => {
  try {
    ...
    let question;
    if (id) { question = dbStore.updateQuestion(id, text, Number(order)); }
    else {
    ...
    if (options && Array.isArray(options)) dbStore.saveOptionsForQuestion(question.id, options);
...
app.post('/api/profiles', (req, res) => {
  try {
    ...
    res.json(dbStore.saveProfile({ id, programId, name, icon: icon || '🚀', color: color || 'orange', summary: summary || '', characteristics: Array.isArray(characteristics) ? characteristics : [], gamificationType: gamificationType || '', mainMessage: mainMessage || '', preferredMissions: Array.isArray(preferredMissions) ? preferredMissions : [], rewards: Array.isArray(rewards) ? rewards : [], customReportText: customReportText || '', keyPoints: Array.isArray(keyPoints) ? keyPoints : [], recommendedObjective: recommendedObjective || '' }));
  } catch (error: any) { res.status(500).json({ error: error.message }); }
});
...
app.post('/api/answers', (req, res) => {
  try {
    ...
    if (!userId || !programId || !questionId || !optionId) return res.status(400).json({ error: 'userId, programId, questionId, optionId são obrigatórios' });
...
app.post('/api/user/trigger', (req, res) => {
  try {
    ...
    if (!userId || !programId || !type || intensity === undefined) return res.status(400).json({ error: 'userId, programId, type, intensity are required' });
...
app.get('/api/admin/stats', (req, res) => {
  try {
    ...
    res.json({
      ...
      profileDistribution: Object.entries(profileCounts).map(([n, v]) => ({ name: n, value: v })),
      programDistribution: Object.entries(programCounts).map(([n, v]) => ({ name: n, value: v })),
      recentActivity: results.slice(-5).reverse().map(r => ({
        id: r.id, userName: r.user?.name || 'Anônimo', userEmail: r.user?.email || '-',
        programName: r.program?.name || 'Geral', profileName: r.profile?.name || 'Indefinido', date: r.calculatedAt
      }))
    });
  } catch (error: any) { res.status(500).json({ error: error.message }); }
});
...
app.post('/api/admin/reset', (req, res) => {
  try { dbStore.resetAllData(); res.json({ success: true }); }
  catch (error: any) { res.status(500).json({ error: error.message }); }
});
